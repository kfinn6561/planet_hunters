...
def main(pi):
    ...
    K=5 #initial number of classes (most general set equal to number of people)
    a,b=10.0,100.0 #hyperparameters, not sure what to set these to
    W=random_init(N,K)
    H=deepcopy(W.T)
    beta=np.ones(K)
    I=identity(N)
    ...
    converged=False
    iteration=1
    while iteration<20 and not converged:
    ...
    p.plot(cf)
    p.show()
    return (results,communities,index2id)
